...
# 🎯 5호기 팩트 데이터 장착
APP_NAME = "물타기 계산기 - 주식 코인 평단가 계산" 
PLAY_STORE_URL = "https://play.google.com/store/apps/details?id=com.bonnyday.average"
BASE_KEYWORD = "물타기 계산기 어플"

def get_app_description(url):
    print("🔍 물타기 계산기 플레이스토어 최신 설명 크롤링 중...")
    ...
    return "주식과 코인 투자 시 물타기 후의 평단가를 쉽고 빠르게 계산해 주는 필수 앱입니다."
...
def write_blog_post(keyword, app_desc):
    ...
    prompt = f"""
    너는 주식/코인 투자 전문 블로거이자 IT 어플 리뷰어야. 검색어 '{keyword}'를 주제로 블로그 글을 작성해 줘.
...
def save_post(keyword, content):
    ...
    # 🚨 파일 이름이 겹치지 않게 '-stock-post'로 변경!
    filename = f"{today}-stock-post.md"
    
    # 주식/차트 테마 고화질 이미지 주입
    top_image = "![주식 차트](https://images.unsplash.com/photo-1611974789855-9c2a0a7236a3?ixlib=rb-4.0.3&auto=format&fit=crop&w=800&q=80)\n\n"
...
"""
    ...
    front_matter = f"---\nlayout: post\ntitle: '{keyword} 추천: 주식 코인 평단가 계산 쉽게 하는 법'\ndate: {today}\n---\n\n"
    
    os.makedirs("_posts", exist_ok=True)
    with open(f"_posts/{filename}", "w", encoding="utf-8") as f:
        f.write(front_matter + final_content)
    print(f"🚀 [성공] 사진과 링크가 완벽하게 박힌 물타기 계산기 전용 {filename} 파일 생성 완료!")
...
if __name__ == "__main__":
    ...
    print("✅ 5호기 물타기 계산기 포스팅 팩트 장착 완료!")
